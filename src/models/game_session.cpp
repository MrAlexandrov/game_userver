...
auto GameSession::Introspect() const {
    return std::tie(id, pack_id, state, current_question_index, created_at, started_at, finished_at);
}
...
userver::formats::json::Value Serialize(
    ...
    auto created_at_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        game_session.created_at.time_since_epoch()).count();
    ...
    if (game_session.started_at.has_value()) {
        auto started_at_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
            game_session.started_at.value().time_since_epoch()).count();
    ...
    if (game_session.finished_at.has_value()) {
        auto finished_at_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
            game_session.finished_at.value().time_since_epoch()).count();
...
GameSession Parse(
    ...
    game_session.id = NUtils::StringToUuid(json["id"].As<std::string>());
    game_session.pack_id = NUtils::StringToUuid(json["pack_id"].As<std::string>());
    game_session.state = json["state"].As<std::string>();
    game_session.current_question_index = json["current_question_index"].As<int>();
